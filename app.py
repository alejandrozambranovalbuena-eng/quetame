...
<style>
    ...
    .stButton > button:hover {{ border-color: {ORANGE} !important; color: {ORANGE_DARK} !important; }}
    button[kind="primary"] {{
        background: linear-gradient(135deg, {ORANGE}, {ORANGE_DARK}) !important; color:#fff !important;
...
<div class="hero">
  ...
  <div class="authors">
    {"".join(f'<div class="author-chip"><div class="author-avatar" style="background:{c};">{"".join(w[0] for w in a.split(" —")[0].split()[:2]).upper()}</div>{a}</div>' for a, c in zip(AUTORES, [ORANGE, BLUE, PURPLE, GREEN]))}
  </div>
</div>
""", unsafe_allow_html=True)
