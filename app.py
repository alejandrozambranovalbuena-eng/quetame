...
<style>
    ...
    /* Monitor grande del centro de mando — mockup de PC en 3D (v9.2) */
    .pc-monitor-wrap {{ display:flex; justify-content:center; margin: 6px 0 4px 0; perspective: 1400px; }}
    .pc-monitor {{
        width: 100%; max-width: 800px; background: linear-gradient(160deg, #1b2130, #0c0f16);
        border-radius: 20px; padding: 14px 14px 30px 14px;
        box-shadow: 0 30px 60px rgba(0,0,0,0.45), inset 0 0 0 2px #2c3346;
        transform: rotateX(2.5deg); transform-style: preserve-3d;
    }}
    .pc-monitor .cam {{ width:9px; height:9px; border-radius:50%; background:#3a4256; margin: 0 auto 10px auto; box-shadow: 0 0 6px rgba(120,180,255,0.5); }}
    .pc-monitor .screen {{
        background: radial-gradient(circle at 25% -10%, #123256 0%, #060910 65%);
        border-radius: 12px; padding: 20px 22px; min-height: 250px;
        box-shadow: inset 0 0 50px rgba(0,0,0,0.65), inset 0 0 0 1px #1c2436;
    }}
    .pc-monitor .stand {{ width:70px; height:34px; background:linear-gradient(160deg,#242a3a,#12141c); margin:0 auto; border-radius: 0 0 8px 8px; }}
    .pc-monitor .base {{ width:190px; height:9px; background:#171a22; margin:0 auto; border-radius:6px; box-shadow: 0 6px 14px rgba(0,0,0,0.35); }}
    .hud-header {{ display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; }}
    .hud-title {{ color:#8fd1ff; font-size:11.5px; font-weight:800; letter-spacing:1.6px; }}
    .hud-dot {{ width:8px; height:8px; border-radius:50%; display:inline-block; margin-right:6px; box-shadow: 0 0 8px currentColor; }}
    .hud-clock {{ color:#5fe0a0; font-size:12px; font-weight:700; font-family: 'Courier New', monospace; }}
    .hud-risk-num {{ font-size:52px; font-weight:900; line-height:1; font-family: 'Courier New', monospace; }}
    .hud-risk-label {{ color:#8b93a8; font-size:11px; letter-spacing:1px; margin-top:2px; }}
    .hud-zone-card {{
        background: rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:10px;
        padding:8px 10px; text-align:center;
    }}
    .hud-zone-name {{ color:#c7cde0; font-size:10.5px; font-weight:700; }}
    .hud-zone-val {{ font-size:19px; font-weight:900; font-family:'Courier New', monospace; }}
    .hud-ticker {{
        margin-top:14px; background: rgba(0,0,0,0.35); border-left:3px solid #2fd18a; border-radius:6px;
        padding:8px 12px; color:#b7f5d3; font-size:11.5px; font-family:'Courier New', monospace;
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }}
    .hud-alert {{
        margin-top:12px; background: linear-gradient(90deg, rgba(240,50,60,0.25), rgba(240,50,60,0.06));
        border:1px solid rgba(240,50,60,0.5); border-radius:8px; padding:10px 14px;
        color:#ffb3b8; font-size:12.5px; font-weight:800; text-align:center;
    }}
...
def render_pc_monitor(ts, r, umbral_ia, zona_sim, disparado, zonas_data, ultimo_log):
    """Mockup grande de un monitor de PC (centro de mando), con efecto 3D vía CSS,
    donde 'llega' toda la información consolidada del simulacro en vivo."""
    color = RED if r > umbral_ia else (AMBER if r > 40 else GREEN)
    dot_color = "#ff5560" if disparado else "#2fd18a"
    estado_txt = "ALERTA ACTIVA" if disparado else "MONITOREO NORMAL"

    zonas_html = ""
    for zname, r_zona, col in zonas_data:
        zonas_html += (f'<div class="hud-zone-card"><div class="hud-zone-name">{zname}</div>'
                        f'<div class="hud-zone-val" style="color:{col};">{r_zona:.0f}</div></div>')

    alerta_html = (f'<div class="hud-alert">🚨 ALERTA ROJA ACTIVA — {zona_sim} — protocolo de evacuación en curso</div>'
                   if disparado else "")
      ...
      <div class="pc-monitor">
        ...
        <div class="screen">
          <div class="hud-header">
            <span class="hud-title">🛰️ CENTRO DE MANDO — SISTEMA DE ALERTA TEMPRANA QUETAME</span>
            <span><span class="hud-dot" style="background:{dot_color};color:{dot_color};"></span>
            <span class="hud-clock">{ts}  ·  {estado_txt}</span></span>
          </div>
          <div style="display:flex; gap:26px; align-items:flex-end; flex-wrap:wrap;">
            <div>
              <div class="hud-risk-num" style="color:{color};">{r:.0f}</div>
              <div class="hud-risk-label">ÍNDICE DE RIESGO — {zona_sim.upper()} (UMBRAL {umbral_ia})</div>
            </div>
            <div style="display:flex; gap:10px; flex:1; min-width:220px;">{zonas_html}</div>
          </div>
          {alerta_html}
          <div class="hud-ticker">&gt; {ultimo_log}</div>
        </div>
        <div class="stand"></div>
        <div class="base"></div>
      </div>
...
def avalancha_3d_figure(progreso, key_suffix=""):
    """Visualización ilustrativa de un escenario EXTREMO: una ladera con una colada de
    tierra y lodo avanzando cuesta abajo. 'progreso' va de 0 (nada) a 1 (llega abajo del todo).
    Es una exageración con fines demostrativos, no una predicción geotécnica real."""
    xs = np.linspace(-9, 9, 22)
    ys = np.linspace(0, 70, 46)
    X, Y = np.meshgrid(xs, ys)
    ladera = 6.2 - 0.075 * Y + 0.45 * np.sin(X / 3) + 0.15 * np.sin(Y / 6)

    fig = go.Figure()
    fig.add_trace(go.Surface(
        z=ladera, x=X, y=Y, showscale=False, opacity=1.0, name="Ladera",
        colorscale=[[0, "#6b4a2f"], [0.5, "#8a9b5a"], [1, "#3f7a3a"]],
        lighting=dict(ambient=0.6, diffuse=0.7, roughness=0.9, specular=0.05),
    ))
    if progreso > 0:
        frente_y = 70 - progreso * 70  # el frente de la colada baja a medida que avanza el progreso
        mask = Y >= frente_y
        colada = np.where(mask, ladera + 0.4, np.nan)
        fig.add_trace(go.Surface(
            z=colada, x=X, y=Y, showscale=False, opacity=0.96, name="Colada de tierra y lodo",
            colorscale=[[0, "#5b3a22"], [1, "#8B4513"]],
        ))
    fig.update_layout(
        height=440, margin=dict(l=0, r=0, t=10, b=0), paper_bgcolor="rgba(0,0,0,0)",
        scene=dict(
            xaxis=dict(visible=False), yaxis=dict(visible=False),
            zaxis=dict(visible=False, range=[0, 9]),
            camera=dict(eye=dict(x=0.9, y=-2.6, z=1.35)),
            aspectratio=dict(x=0.75, y=2.4, z=0.5),
        ),
    )
    return fig
...
with tab_sim:
    ...
    with sc6:
        escenario_avalancha = st.checkbox(
            "🏔️ Ver escenario extremo (avalancha en la ladera)",
            value=False,
            help="Muestra una visualización 3D EXAGERADA de cómo se vería una colada de tierra y lodo bajando "
                 "por la ladera si el deslizamiento avanzara sin control. Es ilustrativo, no una predicción exacta.",
        )
    monitoreo_multizona = st.checkbox(
        "📡 Ver monitoreo simultáneo de las 3 zonas del municipio (no solo la zona seleccionada)",
        value=True,
    )
    ...
    if ejecutar:
        ...
        for i in range(n_pasos):
            ...
            zonas_data = []
            for zname in ZONAS.keys():
                factor = 1.0 if zname == zona_sim else 0.45
                r_zona = min(100, r * factor)
                col_zona = RED if r_zona > umbral_ia else (AMBER if r_zona > 40 else GREEN)
                zonas_data.append((zname, r_zona, col_zona))
            ...
            with placeholder_sim.container():
                ...
                st.markdown("**🖥️ Así llega la información al monitor grande del centro de mando:**")
                st.markdown(
                    render_pc_monitor(ts, r, umbral_ia, zona_sim, disparado, zonas_data, log_lines[-1]),
                    unsafe_allow_html=True,
                )
                st.write("")

                if monitoreo_multizona:
                    st.markdown('**📡 Monitoreo simultáneo — las 3 zonas del municipio (así se vería en el panel real):**')
                    zcols = st.columns(3)
                    for zcol, (zname, r_zona, col_zona) in zip(zcols, zonas_data):
                        with zcol:
                            etiqueta = "📍 zona simulada" if zname == zona_sim else "monitoreo pasivo"
                            st.markdown(f"""
                            <div class="sensor-card" style="text-align:center;">
                            <b style="font-size:12.5px;">{zname}</b><br>
                            <span style="font-size:10.5px;color:{TEXT_GRAY};">{etiqueta}</span><br>
                            <span style="font-size:26px;font-weight:900;color:{col_zona};">{r_zona:.0f}</span>
                            <span style="font-size:11px;color:{TEXT_GRAY};">/100</span>
                            </div>
                            """, unsafe_allow_html=True)
                            st.progress(min(1.0, r_zona / 100))
                    st.write("")
                ...
                if escenario_avalancha:
                    progreso_avalancha = 0.0
                    if disparado:
                        pasos_post = i - trigger_step
                        total_post = max(1, (n_pasos - 1) - trigger_step)
                        progreso_avalancha = min(1.0, pasos_post / total_post)
                    st.markdown(f"**🏔️ Escenario extremo — si la ladera colapsara sin control "
                               f"({progreso_avalancha*100:.0f}% de avance):**")
                    st.plotly_chart(avalancha_3d_figure(progreso_avalancha), use_container_width=True, key=f"avalancha3d_{i}")
                    st.caption("⚠️ Visualización ilustrativa y exagerada — no es una predicción geotécnica exacta, "
                               "es para mostrar la magnitud potencial de un evento sin intervención a tiempo.")
